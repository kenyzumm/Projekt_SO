...
void run_producer_loop(int semid, struct shared_data *shm) {
    ...
    while(fgets(buffer, 256, stdin)!=NULL) { // Główna pętla czytania
        ...
        P_EMPTY; // SEM_EMPTY - czekamy na wolne miejsce w buforze
        P_MUTEX; // Wejdź do sekcji krytycznej (MUTEX)
        ...
        V_MUTEX; //Wyjdź z sekcji krytycznej
        V_FULL; //Powiadom P2, że są dane (SEM_FULL)
